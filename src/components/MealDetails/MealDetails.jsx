...
const {id} =useParams()
const {isError , isLoading , data} = useApi(`https://www.themealdb.com/api/json/v1/1/lookup.php?i=${id}` , `meals/${id}`)
...
<div>
    <img className='w-full rounded-2xl mb-8' src={meal.strMealThumb} alt={meal.strMeal}/>
<ul className='flex gap-4 justify-center'>
<li className='bg-red-600  text-white py-2 px-4 rounded-lg '>

  <a  target='_blank' href={meal.strYoutube} className='flex gap-2 justify-center items-center' > <FaYoutube /> youtube </a>
</li>
<li className='bg-secondary  text-white py-2 px-4 rounded-lg '>

  <a  target='_blank' href={meal.strSource} className='flex gap-2 justify-center items-center'><TfiWorld /> source </a>
</li>
</ul>
</div>
...
{ingredientMeasurePairs.map((pair, index) =>   <div key={index} className='flex justify-between p-2 border-b-2 last-of-type:border-b-0 '>
